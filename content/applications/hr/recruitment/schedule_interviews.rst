...
- :guilabel:`Recurrent`: if the meeting should repeat at a selected interval (not typical for a
  ...
  - :guilabel:`Until`: using the drop-down menu, select when the meetings stop repeating. The
    ...
    the date using the calendar pop-up window, or type in a date in a XX/XX/XXXX format.
...
`Hello,`
`Thanks for your application and interest in (company name).`

`I would like to organize a phone interview with you, it should last about 30 min. We'll go through
your resume and motivations to join the (company name) team. You can book a slot in our agenda by
